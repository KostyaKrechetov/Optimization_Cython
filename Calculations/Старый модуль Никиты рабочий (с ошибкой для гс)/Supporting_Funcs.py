...
def DimensionlessGeometry( unit_length, kv_kh, x, y, z, xw , yw , zw , 
    ...
    #n = len(xw)-1
    #ReDim xwd(n):   ReDim ywd(n): ReDim zwd(n)
...
def DefineGeometry(Wellmodel,ReservoirShape, BoundaryType,skin, h, rw, xf, l_hor, 
    ...
    #        ' 5 - Rectangle (image sources)
    #        ' 6 - Circle
    #        ' 7 - Rectangle (Variables separation)
    #'BoundaryType -
    #        '1 - no flow boundary
    ...
    #
    #' D  -  distance to boundary, (in half plane reservoir)
    #' D1 -  distanse to 1st boundary, (in quater plane reservoir)
    #' D2 -  distanse to 2d boundary, (in quater plane reservoir)
    #' Db -  band width (for band reservoir)
...
#    if compl_type == "vert" and (hw_f != 1 or (hw_f == 1 and (zbound_up == "c" or zbound_down == "c"))):
#        compl_type = "vert_pp"
...
def ConvertUnits(Units, t, delta_p, k ,skin, h, ct, mu,  bl, 
    ...
    #'Wellbore storage to m3/atm
    #Cs = Cs * c_ql[Units] / c_p[Units]
